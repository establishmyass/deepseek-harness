...
- `@deepseek-ai/dsh-experimental-tool-task-surface` — 需要 `tools`（[`packages/experimental/tool-task-surface/src/index.ts`](../packages/experimental/tool-task-surface/src/index.ts)）
