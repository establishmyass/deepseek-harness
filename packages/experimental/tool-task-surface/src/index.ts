/**
 * Model-facing publisher of one declarative Task Surface panel: the call ends
 * the turn, and the user's submission arrives as the next ordinary message.
 *
 * This is the first slice of `.agents/notes/proposed/feature/2026-08-04-task-surface.md`:
 * the tool, its bounded V1 subset, and the panel's identity. The Host
 * projection, dismissal event, transactional submission record, and the
 * `conversation.input.dock` editor land in later slices; until then the Web row
 * itself carries the editor and an ordinary user message is the whole
 * submission path.
 *
